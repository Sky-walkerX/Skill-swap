...
export default function BrowsePage() {
    ...
    return dummyUsers.filter(user => {
      ...
      const matchesSkills = selectedSkills.length === 0 || 
        selectedSkills.some(skillId => 
          user.skillsOffered.filter(skill => skill).some(skill => skill.skillId === skillId) ||
          user.skillsWanted.filter(skill => skill).some(skill => skill.skillId === skillId)
        )
  ...
  const uniqueLocations = [...new Set(dummyUsers.map(user => user.location).filter((location): location is string => Boolean(location)))]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mt-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {dummySkills.map(skill => {
              const userCount = dummyUsers.filter(user => 
                user.skillsOffered.filter(s => s).some(s => s.skillId === skill.skillId) ||
                user.skillsWanted.filter(s => s).some(s => s.skillId === skill.skillId)
              ).length
