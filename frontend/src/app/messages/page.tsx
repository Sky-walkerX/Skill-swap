...
export default function MessagesPage() {
  ...
  const conversations = dummyUsers.filter(user => 
    user.userId !== currentUserId && 
    dummyMessages.some(msg => 
              ...
              <div className="p-4 border-b border-border bg-card flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <Avatar
                    ...
                    fallback={selectedUserData.name.split(' ').map(n => n[0]).join('')}
                    className="w-10 h-10"
                  />
                  <div>
                    <h2 className="font-semibold text-foreground">{selectedUserData.name}</h2>
                        ...
                        {!isOwnMessage && (
                          <div className="flex items-center space-x-2 mb-1">
                            <Avatar
                              ...
                              fallback={sender?.name?.split(' ').map(n => n[0]).join('') || 'U'}
                              className="w-6 h-6"
                            />
