			...
			<div className="max-w-md w-full space-y-8">
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Step {step} of 3:{' '}
							{step === 1 ? 'Basic Information' : step === 2 ? 'Skills' : 'Review'}
						</CardDescription>
					</CardHeader>
					<CardContent>
								...
								<div className="space-y-6">
									<div>
										...
										<p className="text-sm text-muted-foreground mb-3">
											Select the skills you&apos;re confident teaching to
											others
										</p>
										<div className="grid grid-cols-2 gap-2">
											{dummySkills.map((skill) => (
												<button
													...
													type="button"
													onClick={() =>
														toggleSkill(skill.skillId, 'offered')
													}
													className={`p-3 text-left rounded-lg border transition-colors ${
														formData.skillsOffered.includes(
															skill.skillId
														)
										...
										<div className="grid grid-cols-2 gap-2">
											{dummySkills.map((skill) => (
												<button
													...
													type="button"
													onClick={() =>
														toggleSkill(skill.skillId, 'wanted')
													}
													className={`p-3 text-left rounded-lg border transition-colors ${
														formData.skillsWanted.includes(
															skill.skillId
														)
															? 'border-primary bg-primary/10'
															: 'border-border hover:border-primary/50'
								...
								<div className="space-y-4">
									...
									<div>
										<h4 className="font-medium mb-2">
											Skills Offered ({formData.skillsOffered.length})
										</h4>
										<div className="flex flex-wrap gap-1">
											{formData.skillsOffered.map((skillId) => {
												const skill = dummySkills.find(
													(s) => s.skillId === skillId
												);
												return skill ? (
									...
									<div>
										<h4 className="font-medium mb-2">
											Skills Wanted ({formData.skillsWanted.length})
										</h4>
										<div className="flex flex-wrap gap-1">
